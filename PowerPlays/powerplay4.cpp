...
void levelUp(Object& player)
{
	...
	player.health = ((int)randomHealth(engine));
	player.strength = ((int)randomStrength(engine));
	std::uniform_int_distribution<int> newitemgen(1, 4);
	
	Item newitem;
	newitem.clasification = Item::Type(newitemgen(engine) - 1);
	std::normal_distribution<double> itembonus(player.level, player.level / 3);
	newitem.bonusValue == itembonus(engine);
	std::cout << "You found a new item! It looks to be a: " << std::endl;

	auto it = player.inventory.find(newitem.clasification);
	if(it != player.inventory.end())
	{
		if (newitem.bonusValue > it->second.bonusValue)
		{
			it->second = newitem;
		}
	}
	...
	{
		player.inventory.emplace(newitem.clasification, newitem);
	}
...
int calculateAC(const Object& object)
{
	...
	else
	{
		...
		{
			if (item.first == Item::Type::shield || item.first == Item::Type::armor)
			{
				totalbonus = totalbonus + item.second.bonusValue;
			}
		}
		return totalbonus;
	}
...
int attack(const Object& object)
{
	...
	else
	{
		for (const auto item : object.inventory)
		{
			...
			{
				potentialDamage = potentialDamage + item.second.bonusValue;
			}
		}
		return potentialDamage;
	}
	printName(object);
	std::cout << " deals " << potentialDamage;
}
